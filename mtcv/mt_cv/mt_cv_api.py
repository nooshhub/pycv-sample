...
def hot_cold(input_data: InputData, clean_image: bool = True):
    ...
    # 根据输入数据，生成图片
    image_folder, image_path, color_id_dict, image_with_contour_path, input_land_cnts = img_gen.generate(input_data)

    # 生成冷热地块数据
    scale = input_data.scale
    hot_cold_data, image_width_height = hcp.process(image_folder, image_path, color_id_dict, image_with_contour_path,
                                                    input_land_cnts, scale.pixel, scale.km, debug=not clean_image)
